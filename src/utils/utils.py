...
def get_test_loss_acc(net, loss_function, x_data, y_data, test_split=1):
    ...
    for step, (x,y) in enumerate(data_loader):
        with torch.no_grad():
            ...
            pred_bc = torch.max(output_bc, 1)[1].data.cpu().squeeze()
...
def load_models_save_log(network, EPOCH, test_x, test_y, nb_classes,
    ...
    plot_epochs_metric(history, output_directory + 'epochs_acc.png', 'acc')
    ...
    for step, model_id in enumerate(model_save_ids):
        ...
        if save_best_train_log:
            ...
            predict_best_train_net, _, _ = model_predict(best_train_net.cpu(), test_x, test_y, test_split)
        ...
        if save_best_test_log:
            ...
            predict_best_test_net, _, _ = model_predict(best_test_net.cpu(), test_x, test_y, test_split)
